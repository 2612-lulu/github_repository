...
type TXInput struct {
	Txid      []byte                     `json:"id"`    // 引用交易ID，一个输入必须引用一个输出
	Vout      int                        `json:"idout"` // 引用的交易输出编号，引用的交易中具体的某一个output
	Signature uss.USSToeplitzHashSignMsg `json:"uss"`   // 签名，花钱的人要证明这些钱是属于它的，会在交易确认的时候，校验这个签名
