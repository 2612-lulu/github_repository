...
func (node *Node) getAndGenTx(writer http.ResponseWriter, request *http.Request) {
	...
	qbtools.Init_log(NODE_LOG_PATH + "listenHttp_" + node.Node_name + ".log")
	log.SetPrefix(string(node.Node_name[:]) + "listen-tx_message")
...
func (node *Node) getTranscation(writer http.ResponseWriter, request *http.Request) {
	...
	qbtools.Init_log(NODE_LOG_PATH + "listenHttp_" + node.Node_name + ".log")
	log.SetPrefix(string(node.Node_name[:]) + "-listentranscation")
