...
func (node *NodeConsensus) setRoute() {
	http.HandleFunc("/request", node.getRequest)
...
// 参数：
// 返回值：无
func (node *NodeConsensus) getRequest(writer http.ResponseWriter, request *http.Request) {
	var msg block.Block
	...
	log.SetPrefix(string(node.Node_name[:]) + "-block")
	log.Println("receive a block message")
}
...
// 参数：无
// 返回值：无
func (node *NodeConsensus) httplisten() {
	url := node.Node_consensus_table[node.Node_name]
	fmt.Printf("Consensus will be started at %s...\n", url)
...
// 参数：区块数组[]*block.Block
// 返回值：处理错误[]error
func (node *NodeConsensus) resolveRequestMsg(msgs []*block.Block) []error {
	errs := make([]error, 0)
...
func (node *NodeConsensus) resolveRequest(msgs *block.Block) error {
