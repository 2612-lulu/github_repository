...
func (client *Client) getTXmessage(writer http.ResponseWriter, request *http.Request) {
	...
	qbtools.Init_log(LOG_PATH + "listenHttp_" + client.Client_name + ".log")
	log.SetPrefix(string(client.Client_name[:]) + "receive tx_message")
...
func (client *Client) getReply(writer http.ResponseWriter, request *http.Request) {
	...
	qbtools.Init_log(LOG_PATH + "listenHttp_" + client.Client_name + ".log")
	log.SetPrefix(string(client.Client_name[:]) + "receive tx_result")
